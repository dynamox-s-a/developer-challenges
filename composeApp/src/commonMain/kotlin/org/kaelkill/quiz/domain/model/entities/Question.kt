...
data class Question (val id: QuestionId, val statement: QuestionStatement, val options: List<AnswerOption>) {
    companion object {
        fun create(id: String, statement: String, options: List<String>): Result<Question> {
            val uniqueOptions = options.distinct()
            if (uniqueOptions.size != 5) return Result.failure(IllegalArgumentException("A question must have exactly 5 unique options"))
