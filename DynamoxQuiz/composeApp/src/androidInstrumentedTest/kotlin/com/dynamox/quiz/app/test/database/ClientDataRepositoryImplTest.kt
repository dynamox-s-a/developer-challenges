...
class ClientDataRepositoryImplTest : AutoCloseKoinTest() {
    ...
    @Before
    fun setup() {
        _root_ide_package_.org.koin.core.context.startKoin {
            modules(
                module {
                    single<SqlDriver> {
                        val context =
    ...
    @Test
    fun loadUserEntity_and_loadUser_returnsPersistedData() =
        runTest {
    ...
    @Test
    fun createUserEntity_respectsUniqueConstraint() =
        runTest {
            val userLower =
                Dummy.user(email = "user@test.com")
            TestCase.assertTrue(repo.createUserEntity(userLower).isSuccess)

            val userDuplicate =
                Dummy.user(email = "user@test.com")
            TestCase.assertTrue(
                repo.createUserEntity(
                    userDuplicate
                ).isFailure
            )
        }
    ...
    @Test
    fun updateUserEntity_preservesScores_andUpdatesFields() =
        runTest {
            val user = createAndSaveUser()
            ...
            assertEquals(
                beforeUpdateEntity.createdAt,
                afterUpdateEntity.createdAt
            )
            TestCase.assertTrue(afterUpdateEntity.lastModified >= beforeUpdateEntity.lastModified)

            val remainingScores = repo.loadUserQuizScores(user.id).getOrThrow()
            assertEquals(2, remainingScores.size)
            TestCase.assertTrue(remainingScores.any { it.id == score1.id })
            TestCase.assertTrue(remainingScores.any { it.id == score2.id })
        }

    @Test
    fun updateUserEntity_failsWhenUpdatingToExistingEmail() =
        runTest {
    ...
    @Test
    fun deleteUser_marksAsDeleted_andRemovesFromUserBestScoreView() =
        runTest {
            val user = createAndSaveUser()
            createAndSaveScore(userId = user.id, score = 42, timeTaken = 1234)

            TestCase.assertTrue(repo.deleteUser(user.id).isSuccess)

            val entityAfterDelete = repo.loadUserEntity(user.id).getOrThrow()
            TestCase.assertTrue(entityAfterDelete.deleted)

            val rowsInView = repo.loadTopQuizScores().getOrThrow()
            TestCase.assertTrue(rowsInView.isEmpty())
        }

    @Test
    fun emailIsAvailable_trueThenFalseAndStillFalseAfterSoftDelete() =
        runTest {
            val email = "test@test.com"
            TestCase.assertTrue(
                repo.emailIsAvailable(email).getOrThrow()
            )
    ...
    @Test
    fun saveQuizScore_and_loadQuizScore_roundTrip() =
        runTest {
            val user = createAndSaveUser()
    ...
    @Test
    fun saveQuizScore_failsForUnknownUser_dueToForeignKey() =
        runTest {
            val result =
                repo.saveQuizScore(Dummy.score())
            TestCase.assertTrue(result.isFailure)
        }

    @Test
    fun loadTopQuizScores_returnsBestPerUser() =
        runTest {
            val user1 = createAndSaveUser()
            val user2 = createAndSaveUser()
            ...
            TestCase.assertTrue(topScores.any { it.id == user1BestScore.id })
    ...
    fun loadUserQuizScores_appliesSortingAndLimit() = runTest {
        ...
        assertEquals(
            userScores.map { it.id },
            listOf(scoreNewest.id, scoreMiddle.id, scoreOlder.id)
        )
    }
    ...
    fun deleteQuizScore_softDeletePromotesNextBestInView() = runTest {
        ...
        TestCase.assertTrue(repo.deleteQuizScore(score1.id).isSuccess)
    ...
    suspend fun createAndSaveScore(
        id: Uuid = Uuid.Companion.random(),
        userId: Uuid = Uuid.Companion.random(),
        score: Int = Random.Default.nextInt(11),
        timeTaken: Long = Random.Default.nextInt(2, 6).minutes.inWholeMilliseconds,
