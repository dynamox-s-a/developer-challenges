...
def test_create_time_series(session):
    ...
    assert new_series.id is not None
    assert new_series.name == 'Sensor Temperatura'
    assert new_series.created_at is not None


def test_create_time_series_with_data_points(session):
    series = TimeSeries(name='Sensor Pressao')
    session.add(series)
    session.commit()
    ...
    series.data_points.append(point1)
    series.data_points.append(point2)
    session.commit()
...
def test_delete_series_cascades_to_data_points(session):
...
def test_data_point_relationship_to_series(session):
    series = TimeSeries(name='Sensor Teste')
    session.add(series)
    session.commit()

    point = DataPoints(timestamp=datetime(2026, 3, 4, 12, 0, 0), value=50.0)
    series.data_points.append(point)
    session.commit()
    ...
    assert result.series.name == 'Sensor Teste'
