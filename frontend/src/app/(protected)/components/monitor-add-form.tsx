...
export default function MonitorAddForm({ onSubmitSuccess }: { onSubmitSuccess: () => void }) {
    ...
    const machine = machineAndSensorStore((state) => state.machineArray);
    ...
    const ableMachine = machine.filter((machine) => machine.monitors.length < 2)
    ...
    return (
        <form onSubmit={handleSubmit}>
            ...
            <DialogFooter>
                <Button disabled={ableMachine.length === 0} type="submit">Salvar máquina</Button>
            </DialogFooter>
        </form>
    );
}
