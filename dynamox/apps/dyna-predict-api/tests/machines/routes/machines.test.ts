...
describe('POST /v1/machines', () => {
  describe('when machine already exists', () => {
    machineTest(
      'should throw MACHINE_ERR_ALREADY_EXISTS error',
  ...
  describe('when machine does not exist', () => {
    machineTest(
      ...
      async ({ fastify, authenticatedUser, mockMachine }) => {
        ...
        expect(createMachineSpy).toHaveBeenCalledExactlyOnceWith(
          expect.anything(),
          mockMachine.name,
          mockMachine.type,
          authenticatedUser.sub,
  ...
  describe('when another machine with the same name and type already exists', () => {
    machineTest(
      ...
      async ({ fastify, fake, authenticatedUser, mockMachine }) => {
        ...
        const findExistingMachineSpy = vi
          .spyOn(machineRepository, 'findExistingMachine')
          .mockResolvedValue({ id: mockMachine.id + 1 });
...
describe('DELETE /v1/machines/:uuid', () => {
  ...
  describe('when machine is found', () => {
    machineTest(
      'should delete the machine',
      async ({ fastify, authenticatedUser, mockMachine }) => {
        const findExistingMachineByUuidSpy = vi
          .spyOn(machineRepository, 'findExistingMachineByUuid')
          .mockResolvedValue(mockMachine);
        const deleteMachineSpy = vi
          .spyOn(machineRepository, 'deleteMachine')
        ...
        expect(deleteMachineSpy).toHaveBeenCalledExactlyOnceWith(
          expect.anything(),
          mockMachine.uuid,
        );
      },
