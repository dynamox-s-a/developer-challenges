...
To get started with both client and server applications, follow these steps from the project root directory:
...
Run both client and server in parallel with hot reload:
```bash
npm run dev
```

This will start:
- **Client**: React application (typically on `http://localhost:3001`)
- **Server**: Node.js API server (typically on `http://localhost:3000`)
